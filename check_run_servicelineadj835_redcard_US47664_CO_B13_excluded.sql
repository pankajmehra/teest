...
Modified: 09/11/2026	PM      47664   Exclude CO/B13 service-line adjustment records from 835 output
        ...
        FROM [dbo].[claim_procedure_ineligible] AS [cpi]
            ...
            left JOIN [dbo].[vw_edit_list] AS [vel]
				...
				WHERE [cpi].[claim_procedure_id] = @claim_procedure_id
                      -- 47664 Exclude the entire CO/B13 service-line adjustment record
                      AND NOT
                      (
                          [cpi].[claim_adjustment_group_code] = 'CO'
                          AND [cpi].[claim_adjustment_reason_code] = 'B13'
                      )
		GROUP BY 
			  [cpi].[claim_procedure_ineligible_id],
		      [cpi].[claim_procedure_id],
