...
Modified: 09/11/2026	PM      47664   Prefer QPA 32BJ over EOB 207 for CO service-line adjustment when both exist
...
BEGIN TRY
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- 47664: If both EOB 207 (CO-B13) and QPA 32BJ (CO-131/N442)
            -- exist for the same service line, QPA 32BJ must drive the 835 CAS.
            -- TOP 1 + explicit ordering also makes the selection deterministic.
            SELECT TOP 1
                   @inelig_gc = [if].[adjustment_group_code],
                   @inelig_arc = [if].[adjustment_reason_code],
                   @inelig_rarc = [if].[remark_code],
            ...
            WHERE [if].[adjustment_group_code] = 'CO'
            ORDER BY CASE
                         WHEN [if].[eob_ud] = 'QPA 32BJ' THEN 0
                         ELSE 1
                     END,
                     [if].[claim_procedure_ineligible_id];
