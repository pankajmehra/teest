USE [mcr_dc_prod]
GO
/****** Object:  StoredProcedure [dbo].[check_run_master_redcard]    Script Date: 8/13/2026 1:54:47 PM ******/
SET ANSI_NULLS ON
GO
...
/** ---------------------------------------------------------------------------------------------------------  
 ...
 Modified 06/11/2026 PS  46925 including claim reversals on EOPs for Check Run Community 65 (Medica UMR)
 ...
 Modified 08/13/2026 PM  41962 Removing claim_id & ud as parameters from check_run_remarkcodedescription_redcard
 Modified 08/18/2026 PS  47283 Allow claim reversals on EOPs for all groups
---------------------------------------------------------------------------------------------------------------------------------------------**/  
-- Local Variables  
----------------------------------------  
  ...
  -----------------------------------------------  
   ...
   -----------------------------------------------  
    ...
    WHERE 
		...
		AND ( cps.show_on_eop = 1 
		      OR cps.show_on_835 = 1 
		      OR ISNULL(cfsm.added_by, '') = 'claim_negate'  -- 06/24/2026 46972 Send claim negates 
		      OR cps.claim_procedure_status_id IN (61, 127)  -- 07/08/2026 46972 Send FADJ Refund/Net Refund 
			)  
       ...
       begin   
        if @save_suppress_eob = 0 -- so the eob was printed   
         begin   
          ...
          @modified_user_id ,  
          @voucher_id ,  
          --@last_claim_id ,  --removing these AJ 41962
          --@claim_ud ,  
          @doc_type ,  
          @doc_id   ,  
          @line_number  output   
  ...
  -- incriment the doc counter   
   ...
   if @doc_type = 'EOB' -- force a cover sheet  
    set @cForceACoverSheet = 'T'  
       ...
       -- 46449 Populate cPayeeNPI using the same rules as Record 06 cPayeeNPI
   SET @payee_npi = ''
   IF @doc_type IN ('EOB', 'EOP')
   BEGIN
       SELECT TOP 1
...
-- the end of the cursor /record reads for this voucher. If there was a check create the record now.   
 if @doc_type in ('EOB','EOP')  
  ...
  begin   
   ...
   @modified_user_id ,  
   @voucher_id ,  
   --@claim_id ,  --removing these AJ 41962
   --@claim_ud ,  
   @doc_type ,  
   @doc_id   ,  
   @line_number  output   
